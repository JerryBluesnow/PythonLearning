...
def printRecordList(RecordList): 
    for list_item in RecordList:
        print sys._getframe().f_code.co_name, ":", list_item
    return 
...
def is_valid_cdr_file_string(str):
    ...
    startpos = str.find('.')
    endpos = str.rfind('+')
    print sys._getframe().f_code.co_name, ":", startpos, endpos
    if startpos == -1:
        return False
    if endpos == -1: 
        return False
...
def calculateCallRecordDuriationSum(durationRecordList): 
    ...
    for listitem in durationRecordList:
        if len(listitem) == 4:
            ...
            print sys._getframe().f_code.co_name,"VN_ID-TG_ID:", key
...
def decodeRecord(RecordItem):
    ...
    if is_valid_date(start_time) == False or is_valid_date(end_time) == False:
        print "+drop the record, because of error timestamp in CDR decode files"
        return None
    # FIX BUG-13169: END
...
def CDRFileFormatString_toDatetime(filename):
    ...
    print sys._getframe().f_code.co_name, "input filename:", filename
...
def checkFileInTimeRange(starttime, endtime, inputfilename):
    ...
    if (starttimeDatetime < fileDatetime < endtimeDatetime) or fileDatetime == endtimeDatetime:
        print sys._getframe().f_code.co_name, inputfilename, "in range"
        return True
    else:
        print sys._getframe().f_code.co_name, inputfilename, "out of range"
        return False
...
def main():
    ...
    if cmp(options.endtimestamp, options.starttimestamp) != 1:
        print 'ERROR: input endtimestamp is not later than start timestamp, return...'
        return -1
    ...
    while end_hour_stamp > hour_stamp:
        cdr_process_per_hour_main(hour_stamp, options.path)
        hour_stamp = hour_stamp + datetime.timedelta(hours=1)
        file_count = file_count + 1

    print '+ Generate', file_count, 'csv files...'
...
def cdr_process_per_hour_main(_date_stamp, _path):
    ...
    print '+++++++++++++++++++++++++++++++START+++++++++++++++++++++++++++++++++++++++++++'
    print 'process cdr files with timestamp in the set (' + starttime + ',' + endtime + ']'
    ...
    for filename in filelist:
        ...
        if checkFileInTimeRange(starttime, endtime, filename):
            ...
            if _path[len(_path) - 1] == '/':
    	        os.system('asnccflsearch '+ _path + filename)
            else:
    	        os.system('asnccflsearch '+ _path + '/' + filename)
            ...
            for filename in filelist:
                ...
                if ".details" in filename:
                    ...
                    continue
                if checkFileInTimeRange(starttime, endtime, filename):
                    print "process the decoded file", filename
    ...
    if os.path.exists(tosavedFilename + '.raw.csv'):
        print tosavedFilename + '.raw.csv', 'exist, delete it'
        os.system('rm ' + tosavedFilename + '.raw.csv')
    
    if os.path.exists(tosavedFilename + '.csv'):
        print tosavedFilename + '.csv', 'exist, delete it'
