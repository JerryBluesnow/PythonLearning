...
#-------------------------------------------------------------------------------------------#
# version 1.5 
#             1. work out all files Queue
#             2. use queue to parse CDR in Queue 
#             3. python ~/cdr_to_csv.py -d 2019/02/12 -s 00:00:00 -e 23:59:59 -t /export/home/lss/asnccflsearch_private
#-------------------------------------------------------------------------------------------#
#-------------------------------------------------------------------------------------------#
# version 1.4 
#             1. multithread won't decrease the calculation peroid in version1.3. 
#             2. use multiprocess instead  
#             3. will not save .raw.csv files anymore for disk usage consideration
#             4. change asnccflsearch_private decoded directory from
#                '/export/home/lss/ccfl_decode' to '/storage/ccfl_decode'
#-------------------------------------------------------------------------------------------#
#-------------------------------------------------------------------------------------------#
# version 1.3 
...
cdr_decoded_directory = '/storage/ccfl_decode'
...
def is_valid_cdr_file_string(str):
    ...
    CDR_LOG.debug("Verify File Name %s if in good format: mum01-4_-_24.20180816_-_1801+0530", str)

    startpos = str.find('.')
    endpos = str.rfind('+')

    CDR_LOG.debug("%s: Date information in filename, startpos: %d, endpos:%d",sys._getframe().f_code.co_name, startpos, endpos)
    if startpos == -1:
        return False
    if endpos == -1: 
        return False
...
def decodeRecord(RecordItem):
    ...
    CDR_LOG.debug("VN=%d TG=%d, incoming CallDuration=%d", incomingVN, incomingTG, callDuriation)
    CDR_LOG.debug("VN=%d TG=%d, outgoing CallDuration=%d", outgoingVN, outgoingTG, callDuriation)
...
def CDRFileFormatString_toDatetime(filename):
    ...
    CDR_LOG.debug("%s input filename: %s", sys._getframe().f_code.co_name, filename)
...
def main():
    ...
    cdr_raw_files_dict_per_hour = {}
    while end_hour_stamp > hour_stamp:
        cdr_raw_files_dict_per_hour[hour_stamp] = Manager().Queue()
        hour_stamp = hour_stamp + datetime.timedelta(hours=1)

    classify_cdr_files(cdr_raw_files_dict_per_hour, options.path)

    PROCESS_COUNT = 4
    cdr_process_pool = Pool()
    hour_stamp = start_hour_stamp
    for key in cdr_raw_files_dict_per_hour:
        cdr_process_pool.apply_async(func=cdr_process_per_hour_main, args=(cdr_raw_files_dict_per_hour, hour_stamp, options.path,))
        hour_stamp = hour_stamp + datetime.timedelta(hours=1)
    cdr_process_pool.close()
    cdr_process_pool.join()
    cdr_process_pool.terminate()

    while True:
        terminate_flag = 0
        time.sleep(20)
        for file_queue in cdr_raw_files_dict_per_hour.values():
            if not file_queue.empty():
                terminate_flag = terminate_flag + 1
            else:
                terminate_flag = terminate_flag + 0
        if terminate_flag == 0:
            cdr_process_pool.terminate()
            break
        # check disk 
...
def classify_cdr_files(_cdr_raw_files_dict_per_hour, _path):
    ...
    for filename in filelist: 
        ...
        if ".details" in filename:
            ...
            continue
        if ".decoded" in filename:
            CDR_LOG.debug("%s will not be processed as it is detailed file", filename)
            continue
        
        file_datetime = CDRFileFormatString_toDatetime(filename).replace(minute=0, second=0,microsecond=0)

        try:
            _cdr_raw_files_dict_per_hour[file_datetime].put(filename)
            CDR_LOG.debug("filename [%s], put it into queue", filename)
        except:
            CDR_LOG.debug("filename [%s], keys not found in dict", filename)

    return

def cdr_process_per_hour_main(_cdr_raw_files_dict_per_hour, _date_stamp, _path):
    ...
    if _cdr_raw_files_dict_per_hour[_date_stamp] is None:
        CDR_LOG.warning('no cdr files with timestamp in the set (%s, %s]', starttime , endtime)
        return

    durationOverAllList = []
    while not _cdr_raw_files_dict_per_hour[_date_stamp].empty():
        filename = _cdr_raw_files_dict_per_hour[_date_stamp].get()
        CDR_LOG.info("process the file: %s", filename)
        ret_output = subprocess.check_output([AsnCCFLSearch_Tool + ' ' + _path + '/' + filename], shell=True)
        CDR_LOG.info('decode oyutput: %s', ret_output)
        resultRecordList = loadCDRdecodedFile( cdr_decoded_directory + '/' + filename + '.decoded')
        
        retvalue = decodeRecordList(resultRecordList, durationOverAllList)
        # remove all files in the decoded directory
        CDR_LOG.debug("clearn up decoded file: %s, %s", filename + '.decoded', filename + '.details')
