...
def parseHTTPHeader(file, topline, param, header):
    ...
    with open(file) as file_obj:
        contents = file_obj.read()
        http_content = re.findall(match_str, contents, re.S|re.M)
        if not http_content:
            print("Unmatched")
            sys.exit()

	header_content = re.findall(header + ':(.*$)', http_content[0], re.M)
        print(header_content[0].replace('\r', '').replace('\n', '').replace(' ', ''))

if __name__ == '__main__':
    file, topline, param, header = menu()
    
    http_raw_file = file + '.http'
    if not os.path.exists(http_raw_file):
        os.system('grep -A100 "HTTP\/.\.." '+ file + ' > ' + file + '.http')

    if header:
        parseHTTPHeader(http_raw_file, topline, param, header)
        sys.exit(0)
    
    parseHTTPJsonContent(http_raw_file, topline, param)
    sys.exit(0)
